...
// разделами НЕ являются, поэтому applyNavOrder() их пропускает (иначе якорь
// «первый не-раздел» нашёлся бы раньше разделителя низа сайдбара).
const NAV_WRAPPERS = ['nav-estimates-wrap'];
...
function applyNavOrder(order) {
    ...
    const skipIds = [...TAB_BUTTONS_MAP.map(([btnId]) => btnId), ...NAV_WRAPPERS];
    const anchor = [...container.children].find(el => !skipIds.includes(el.id));
    if (!anchor) return;
    ...
    order.forEach(tabId => {
        ...
        // «📐 Сметы» переносим ВМЕСТЕ с обёрткой её меню: кнопку одну двигать
        // нельзя — меню оторвалось бы от неё и позиционировалось бы не там.
        const btn = document.getElementById(tabId === 'estimates' ? NAV_WRAPPERS[0] : entry[0]);
...
function applyPermissionsToUI() {
    TAB_BUTTONS_MAP.forEach(([btnId, tabId]) => {
        ...
        // Подпись ставим в её узел, если он есть: у кнопки «📐 Сметы» внутри ещё
        // стрелка меню, и textContent стёр бы её.
        const label = getNavLabel(tabId);
        if (label) {
            const node = btn.querySelector('[data-nav-label]') || btn;
...
// Меню кабинета и меню кнопки «📐 Сметы» на ПК раскрываются ВБОК от колонки меню
// и вниз от своей кнопки. Абсолютным позиционированием этого не сделать: у
// сайдбара `lg:overflow-y-auto` (на коротком экране он прокручивается), а
// прокрутка обрезает всё, что выходит за её границы — от меню с `left: 100%`
// оставалась бы полоска в 4 пикселя. Поэтому на ПК оба меню позиционируются
// `fixed` (css/style.css), а координаты считает placeSideMenu() по кнопке и краю
// колонки: место общее, отличается только пара «меню → кнопка».
const MENU_SIDE_GAP = 12;   // зазор между колонкой меню и боковым меню, px
...
/**
 * Ставит меню вбок от колонки меню и вниз от своей кнопки (ПК).
 ...
 */
function placeSideMenu(menuId, btnId) {
...
function placeProfileMenu() {
    placeSideMenu('profile-menu', 'profile-btn');
}

function placeEstimatesMenu() {
    placeSideMenu('estimates-menu', 'btn-estimates');
}

/** Позиция обоих боковых меню — одна функция для resize и scroll. */
function placeSideMenus() {
    placeProfileMenu();
    placeEstimatesMenu();
}
...
/**
 * Кнопка «📐 Сметы»: раскрывает меню раздела (создать смету, список смет,
 * справочники, клиенты, настройки). Само переключение раздела делает второе
 * действие кнопки — `data-after="switchTab"` в index.html, поэтому одно нажатие
 * и открывает раздел, и показывает меню.
 */
export function toggleEstimatesMenu() {
    const menu = document.getElementById('estimates-menu');
    if (!menu) return;
    menu.classList.toggle('hidden');
    placeEstimatesMenu();
}

/** Закрывает меню раздела: его пункты вызывают это после своего действия. */
export function closeEstimatesMenu() {
    const menu = document.getElementById('estimates-menu');
    if (menu) menu.classList.add('hidden');
}

window.toggleProfileMenu = toggleProfileMenu;
window.toggleEstimatesMenu = toggleEstimatesMenu;
window.closeEstimatesMenu = closeEstimatesMenu;
...
document.addEventListener('click', (e) => {
    // Оба боковых меню закрываются одинаково: клик вне самой кнопки и вне меню.
    [['profile-menu', 'profile-btn'], ['estimates-menu', 'btn-estimates']].forEach(([menuId, btnId]) => {
        const menu = document.getElementById(menuId);
        const btn = document.getElementById(btnId);
        if (!menu || !btn) return;
...
// Меню кабинета и меню «📐 Сметы» на ПК позиционированы `fixed` (вбок от колонки,
// см. выше), а `fixed` не следует за прокруткой — поэтому место пересчитывается
// при изменении окна и при прокрутке. Прокрутка ловится на фазе перехвата:
// событие `scroll` не всплывает, но при перехвате доходит и от самой колонки меню.
window.addEventListener('resize', placeSideMenus);
document.addEventListener('scroll', placeSideMenus, true);
