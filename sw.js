...
//   r3 — вторая проверка на объекте (та же версия 2.11.0):
//        1) текст в PDF сметы прижимался к НИЖНЕЙ линии ячейки. Причина была
//           не в стилях: html2canvas 1.4.1 измеряет «базовую линию» шрифта
//           своим пробником (div → img), а preflight Tailwind делает картинки
//           блочными — пробник вставал на отдельную строку, и весь текст
//           печатался на строку ниже. Добавлен js/utils.js → renderPdfCanvas:
//           на время снимка пробник снова инлайновый, текст ложится туда, где
//           его поставил браузер. Тот же обход применён и к другим снимкам
//           (PDF сметы объекта, график Ганта);
//        2) подписи в файлах (PDF и Excel) печатаются на языке из «Настроек»:
//           русский интерфейс — русский документ, украинский — украинский.
//           Таблица подписей DOC_TEXT в js/modules/estimate-doc.js; переведены
//           и колонки, и итоги, и имя листа Excel. Данные сотрудника (названия
//           работ, материалов, разделов, заказчик, примечания) не переводятся.
