...
//  r15 — меню перестало быть «залитым» фирменным цветом: поверхность шапки (на
//        ПК — сайдбар слева) белая, а цвет схемы остался рамкой, разделителем и
//        акцентом. Разделы — белые кнопки с рамкой в цвете схемы, выбранный
//        раздел выделен подложкой схемы и акцентной полосой слева, кнопки-
//        действия («Заказ материалов», «Финансовые запросы») — заливка цветом
//        схемы. От страницы меню отделяет полоса в цвете схемы: снизу на
//        телефоне, справа на ПК (index.html → .nav-surface, .nav-divider,
//        .nav-chip, .nav-cta в css/theme.css). Схема по умолчанию — прежняя
//        «Красная» (#c8102e).
//        Оболочка переустанавливается потому, что изменились index.html,
//        css/theme.css, css/tailwind.css (классы новой шапки), js/theme.js и
//        js/main.js — строка-отпечаток в конце css/tailwind.css.
//  r16 — новый логотип в меню приложения и меню кабинета сбоку. В шапке (на ПК —
//        в колонке слева) стоит широкая картинка logo2.png: знак FD, название и
//        подпись одним файлом; квадратный logo.png остался источником значков
//        ярлыка, вкладки (.ico) и обложек инструкций, поэтому в офлайн-оболочку
//        добавлен второй файл. Меню кабинета на ПК раскрывается ВБОК от колонки
//        и вниз (раньше — вверх, внутри колонки): само меню позиционируется
//        `fixed`, а координаты считает placeProfileMenu() (js/main.js) по кнопке
//        и краю колонки, поэтому прокрутка сайдбара (`lg:overflow-y-auto`) его
//        больше не обрезает; стрелка на кнопке смотрит вправо-вниз. Телефон и
//        планшет не изменились: там меню по-прежнему открывается вниз (карточкой
//        по центру или под кнопкой).
//        Оболочка переустанавливается потому, что изменились index.html,
...
//   v2.11.0 начинается с r1 — по той же причине.
//   r1 — выпуск v2.11.0: рабочее место ПТО доведено до вида «СметаPRO» —
//        у кнопки «📐 Сметы» появилось выпадающее меню (создать смету, список,
//        справочники, клиенты, настройки), в справочниках — папки и подпапки,
//        в окне «📥 Экспорт документа» — выбор типа документа, вида кошторису
//        (6-ти и 9-ти графка), кольору шапки и формата (PDF/Excel), а документы
//        теперь собираются ОДНОЙ моделью (js/modules/estimate-doc.js) для обоих
//        форматов. Оболочка обновилась потому, что изменились index.html,
//        js/main.js, js/config.js, js/modules/estimates.js,
//        js/modules/estimate-catalog.js, js/modules/estimate-doc.js, css/style.css
//        и css/tailwind.css (строка-отпечаток в конце файла). Библиотека выгрузки
//        в Excel заменена на совместимую xlsx-js-style: бесплатный SheetJS
//        заливку ячеек при записи молча выбрасывает, и «колір шапки» работал бы
//        только в PDF.
//   r2 — правки по итогам первой проверки на объекте (та же версия 2.11.0):
//        1) рамка таблицы в Excel была неполной — у объединённых ячеек Excel
//           рисует рамку по краям диапазона, поэтому покрытые ячейки теперь
//           получают стиль ячейки-источника (шапка 9-ти графки, «Всього по
//           розділу:»); 2) текст в рамках документа выровнен по центру
//           (vertical-align:middle) — на печати высокая ячейка прижимала
//           подпись к верхнему краю; 3) в справочнике нет отдельной вкладки
//           «Разделы»: папки и подпапки живут в «Работы» и «Материалы», и
//           позиция создаётся кнопкой «➕» прямо у папки — прайс заполняется
//           сверху вниз, от раздела к работе.
