...
export default function SectionModal({ section, onSave, onClose }) {
  ...
  const loadParents = async () => {
    try {
      // Только родительские разделы (parentId=null)
      const all = await getSections({ parentId: 'null' })
      // Исключаем самого себя (если редактируем)
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-800">
            {section ? 'Редагувати розділ' : 'Новий розділ'}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Наприклад: Штукатурні роботи"
              autoFocus
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Якщо обрати батька — цей розділ стане підрозділом.
            </p>
          </div>
